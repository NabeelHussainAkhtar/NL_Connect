...
async function testKey(key) {
  ...
  try {
    ...
    return { key, status: "OK", models: res.data.length };
  } catch (e) {
    return { key, status: "FAIL", error: e.message };
  }
}

async function main() {
  console.log("Testing keys with OpenAI SDK...");
  for (const key of keys) {
    const result = await testKey(key);
    console.log(`${result.key}: ${result.status} ${result.error || `(Found ${result.models} models)`}`);
    if (result.status === "OK") {
      console.log("!!! WORKING KEY FOUND !!!", result.key);
      break;
    }
  }
}
